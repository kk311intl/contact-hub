...
export function renderHome(config, lang, origin, visitorCount = null, legacyPublicUi = false) {
  ...
  return documentShell({
    ...
    body: `
      ...
      <button type="button" class="redirect-notice" data-redirect-notice aria-live="polite" hidden><span class="redirect-message" data-redirect-message></span><svg class="redirect-border" aria-hidden="true" focusable="false"><rect x="1" y="1" rx="23" pathLength="100"/></svg></button>
      <script type="module" src="/app-v3.js"></script>
      <script type="module" src="/inapp-redirect.js"></script>`
  });
}
      ...
      <div class="admin-shell">
        ...
        <main id="admin-main" class="admin-main">
          ...
          <form data-admin-form autocomplete="off" novalidate>
            ...
            <section class="editor-section" data-section="settings"><div class="section-heading"><h2>${ui.settings}</h2><p>${ui.settingsHelp}</p></div><div class="form-card">${field("siteTitle", ui.siteTitle, config.settings.siteTitle, "text", 120, ui.siteTitleHint)}${field("canonicalUrl", ui.canonical, config.settings.canonicalUrl, "url", 2048, ui.canonicalHint)}<label class="switch"><input type="checkbox" name="autoRedirectEnabled"${config.settings.autoRedirectEnabled ? " checked" : ""}><span aria-hidden="true"></span><b>${ui.autoRedirect}</b></label><label class="field"><span>${ui.autoRedirectDelay}</span><input type="number" name="autoRedirectSeconds" min="1" max="10" step="1" value="${config.settings.autoRedirectSeconds}"><small>${ui.autoRedirectHelp}</small><small data-redirect-apps>${ui.autoRedirectApps}</small></label></div><div class="footer-settings"><div class="section-heading"><h3>${ui.footerLink}</h3><p>${ui.footerHelp}</p></div><div class="footer-links-editor" data-footer-links-editor></div></div></section></form><section class="editor-section" data-section="settings"><div class="form-card password-card" data-password-panel><div><h3>${ui.changePassword}</h3><p>${ui.passwordHelp}</p></div>${passwordField("currentPassword", ui.currentPassword, "current-password")}${passwordField("newPassword", ui.newPassword, "new-password")}${passwordField("confirmPassword", ui.confirmPassword, "new-password")}<button class="password-button" type="button" data-password-save>${ui.updatePassword}</button></div></section>
        </main>
      </div>
      <div class="toast" role="status" aria-live="polite" data-toast></div>
